...
from whispy_lispy import interpreter, parser
import whispy_lispy.ast
...
class InterpreterTest(unittest.TestCase):
    ...
    def test_assigment_changes_global_scope(self):
        scope = {}
        interpreter.interpret([whispy_lispy.ast.Assign('x', 3)], scope)
        self.assertEqual(scope['x'], 3)

    def test_assignments_change_scope_and_interpreter_returns_value(self):
        scope = {}
        result = interpreter.interpret([whispy_lispy.ast.Assign('x', 6), 4], scope)
