...
from whispy_lispy import ast, exceptions
...
def interpret_apply(tree, scope):
    ...
    if not isinstance(tree[0], ast.Symbol):
        raise exceptions.EvaluationError(
            "Can only evaluate functions, not {}".format(tree[0])
        )
    return scope[tree[0][0]](*[interpret_ast(node, scope) for node in tree[1:]])  # noqa
